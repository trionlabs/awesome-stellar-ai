...
test("reports a missing primary URL without crashing", async () => {
  const catalog = baseCatalog();
  delete catalog.entries[0].url;

  const result = await run({ catalog });
  assertClean(result);
  assert.equal(result.code, 1);
  assert.match(result.output, /must use an HTTPS primary URL/);
});
...
test("warns when a claim about elsewhere cites only the entry's own page", async () => {
  for (const type of ["deployment", "event", "release"]) {
    const catalog = baseCatalog();
    catalog.entries[0].evidence = [{ type, url: "https://example.com/alpha#proof" }];

    const result = await run({ catalog });
    assert.equal(result.code, 0, "self-citation is editorial debt, not a build break");
    assert.match(result.output, /cites nothing outside its own page/);
  }
});

test("does not warn when the artifact is its own reference", async () => {
  // A library or an official guide is the thing being cited, so pointing at it
  // is correct rather than circular. Warning here would bury the real cases.
  for (const type of ["source", "documentation"]) {
    const catalog = baseCatalog();
    catalog.entries[0].evidence = [{ type, url: "https://example.com/alpha#readme" }];

    const result = await run({ catalog });
    assert.equal(result.code, 0);
    assert.doesNotMatch(result.output, /Alpha claims .* but cites nothing/);
  }
});
