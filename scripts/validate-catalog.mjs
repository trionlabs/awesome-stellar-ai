...
// Evidence types that assert something happened away from the project's own
// pages, and therefore need a source the project does not control.
const assertedEvidenceTypes = new Set(["deployment", "event", "release"]);
...
// An entry can fail the HTTPS check above and still reach the own-page checks
// below, so tolerate a missing primary URL instead of crashing on it.
const pageOf = (url) => (typeof url === "string" ? url.split("#")[0] : "");
...
for (const record of catalog.entries) {
  ...
  //
  // Source and documentation evidence is exempt: for a library or an official
  // guide, the artifact is the reference, so linking to it is correct rather
  // than circular. Only claims about something that happened elsewhere need a
  // source the project does not control.
  const asserted = record.evidence.filter((item) => assertedEvidenceTypes.has(item.type));
  const independent = record.evidence.filter(
    (item) => item.url && pageOf(item.url) !== pageOf(record.url),
  );

  if (asserted.length > 0 && independent.length === 0) {
    warnings.push(
      `${record.name} claims ${[...new Set(asserted.map((item) => item.type))].join(" and ")} evidence but cites nothing outside its own page; every link resolves to ${pageOf(record.url)}.`,
    );
  }
