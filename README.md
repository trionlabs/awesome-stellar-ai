...
- [ROZO Checkout](https://github.com/RozoAI/rozo-checkout-skill) - Agent skill and npm CLI that settles a Coinbase Payment Link invoice with Stellar USDC, routing the deposit and its required text memo so services that only accept USDC on Base can be funded from Stellar ([package](https://registry.npmjs.org/@rozoai/checkout/latest)).
